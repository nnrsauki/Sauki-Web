...
export default function UserApp() {
  ...
  const verifyPayment = async (ref = activeRef, isAuto = false) => {
    ...
    // Stop after approx 16 seconds (4 checks * 4s) if it's auto polling
    if (pollCount.current >= 4) {
        clearInterval(pollTimer.current);
        setVerificationStatus('delayed'); // Show "Retry Later" options
        return;
    }
    ...
    try {
      ...
      // ONLY SHOW SUCCESS IF API SAYS SUCCESS (Data Delivered)
      if (data.success) {
        clearInterval(pollTimer.current);
      ...
      // If failed/pending, we keep silent in auto-mode or show error in manual mode
      else {
          if (!isAuto && verificationStatus !== 'delayed') {
             alert(data.error || "Payment not received yet.");
          }
      }
  ...
  const handleGiveaway = () => {
    ...
    window.open(`${ADMIN_NUMBER}?text=${encodeURIComponent(msg)}`, '_blank');
  };
  ...
  const handleTrack = async () => {
    ...
    try {
        ...
        // Fallback: Try Recheck directly if track endpoint fails
        retryTransaction(trackQuery, true);
    }
    setIsLoading(false);
  };
  
  const retryTransaction = async (ref, silent = false) => {
      if(!silent && !confirm("Retry this transaction?")) return;
      setIsLoading(true);
      try {
        const res = await fetch('/api/purchase', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ action: 'recheck', tx_ref: ref })
        });
        const d = await res.json();
        if(d.success) {
            alert("Transaction Successful! Data sent.");
            if(d.message === 'Data Sent!') {
               // Update receipt data if possible, or reload track
               handleTrack(); 
            }
        }
        else {
            if(!silent) alert("Retry Failed: " + (d.error || "Unknown Error"));
        }
  ...
  const downloadReceiptImage = async () => {
    // Show hidden render element
    const element = document.getElementById('receipt-hidden-render');
    element.style.display = 'block';
    
    try {
        // Dynamic import to avoid build errors with 'next/script'
  ...
  const viewReceipt = (t) => {
      setReceiptData({
          ...
          plan: t.plan_id, // This contains the plan name usually
          amount: 'PAID', // Your transaction API needs to return amount to be exact, currently generic
          status: 'Successful'
      });
      setShowReceiptModal(true);
  };
  
  const sendToWhatsApp = (text) => window.open(`${ADMIN_NUMBER}?text=${encodeURIComponent(text)}`, '_blank');
  ...
  return (
    <div className="h-[100dvh] bg-slate-50 flex flex-col max-w-md mx-auto shadow-2xl relative font-sans overflow-hidden">
      ...
      <div className="bg-slate-900 text-white text-[10px] py-2 px-3 flex justify-between items-center shrink-0 z-30 font-medium">
        ...
        <a href={`${ADMIN_NUMBER}`} target="_blank" className="flex items-center gap-1 hover:text-green-300"><WhatsappIcon /> <span>WhatsApp</span></a>
      ...
      {broadcastMsg && <div className="bg-orange-600 text-white text-[11px] font-bold py-1.5 overflow-hidden whitespace-nowrap shrink-0 z-30 shadow-md"><div className="inline-block animate-[marquee_15s_linear_infinite] pl-[100%]">{broadcastMsg} &nbsp;&nbsp;&nbsp;&nbsp; {broadcastMsg}</div></div>}
        ...
        <div className="flex-1 flex flex-col bg-white overflow-hidden">
          ...
          <div className="flex-1 p-6 flex flex-col items-center text-center overflow-y-auto">
            ...
            {verificationStatus === 'polling' && (
                <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex items-center gap-3 w-full mb-6">
            ...
            {verificationStatus === 'delayed' && (
                <div className="bg-orange-50 border border-orange-100 p-4 rounded-xl w-full mb-6 text-left">
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => verifyPayment(activeRef, false)} className="px-3 py-2 bg-orange-200 text-orange-900 rounded-lg text-xs font-bold flex-1 hover:bg-orange-300">Retry Now</button>
                        ...
                        {trackResult.transactions ? trackResult.transactions.map(t => (
                            <div key={t.id} className="border-b border-slate-100 pb-3 mb-3 last:border-0 last:mb-0 last:pb-0">
                                ...
                                <div className="flex justify-end gap-2">
                                    ...
                                    {t.status !== 'success' && <button onClick={()=>retryTransaction(t.reference)} className="px-3 py-1 bg-orange-100 text-orange-600 rounded text-xs font-bold flex items-center gap-1"><RotateCcw size={12}/> Retry</button>}
                                </div>
                            </div>
